...
:ball/sball {{split} buffer all} - редактировать все файлы в списке аргументов Vim (sball откроет в новых окнах)  
...
(\U)/(\L) и (\e)/(\E) - такие же действия как и у \u или \l, до тех пор пока не встретят \e или \E или конец строки замены  
``` 
