...
> Пролог: данная шпаргалка была написана в течение прочтения книги по Vi-подобным языкам. Она никак не относится к официальной рекомендации по изучению редактора.  
...
### БАЗОВЫЕ КОМАНДЫ
```
i[text] {insert} - вставка [text] на позицию курсора
...
Примечание: последние 2 команды полезны при вставке текста из буфера обмена ОС
```
...
:ball или sball {{split} buffer all} - редактировать все файлы в списке аргументов Vim (sball откроет в новых окнах)
:unhide или sunhide - редактировать все загруженные буферы (sunhide откроет в новых окнах)
...
:buffer [n] или sbuffer [num] {{split} buffer} - переход в буфер [n] (sbuffer откроет в новом окне)
:bnext [n] или sbnext [n] {{split} buffer next} - переход в следующий по порядку [n] буфер (sbnext откроет в новом окне)
:bNext [n] или sbNext [n] {{split} buffer next} - переход в [n] предыдущий буфер (sbNext откроет новое окно)
:bfirst или sbfirst {{split} buffer first} - переход к первому буферу (sbfirst откроет новое окно)
:blast или sblast {{split} buffer last} - переход к последнему буферу (sblast откроет новое окно)
:bmod [n] или sbmod [n] {{split} buffer modified}- переход к [n] измененному буферу (sbmod откроет новое окно)
...
> __Не используются в Vim из коробки__
```
...
### БАЗОВЫЕ КОМАНДЫ
```
:source [file] - использовать команды из файла [file]
...
:tag [function name] или ^] - поиск в файле tags местоположение функции [function name] и перемещение курсора на ее определение
