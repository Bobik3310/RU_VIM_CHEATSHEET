...
> Пролог: данная шпаргалка была написана в течение прочтения автором книги по Vi-подобным редакторам. Она никак не относится к официальной рекомендации по изучению редактора. В репозитории содержится pdf версия шпаргалки для желающих распечатать её. По всем улучшениям прошу писать в Discussions.
...
:ball или sball {{split} buffer all} - редактировать все файлы в списке аргументов Vim
...
:buffer [n] или sbuffer [num] {{split} buffer} - переход в буфер [n]
:bnext [n] или sbnext [n] {{split} buffer next} - переход в следующий по порядку [n] буфер
:bNext [n] или sbNext [n] {{split} buffer next} - переход в [n] предыдущий буфер
:bfirst или sbfirst {{split} buffer first} - переход к первому буферу
:blast или sblast {{split} buffer last} - переход к последнему буферу
:bmod [n] или sbmod [n] {{split} buffer modified}- переход к [n] измененному буферу

Примечание:
Команды, начинающиеся на s* открывают новые окна.
