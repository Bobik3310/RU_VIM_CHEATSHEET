...
> В тексте используется обозначения:  
> {...} - расшифровывает мнемонику команды  
> [...] - аргументы команды  
> (...) - значения по-умолчанию или пояснения
...
:ball/sball {split buffer all} - редактировать все файлы в списке аргументов vim (sball откроет в новых окнах) (вспомни про команду args)  
...
:bNext [n]/sbNext [n] {{split} buffer next} - переход в [n] предыдущий буфер (sbNext откроет новое окно)  
...
:bmod [n]/sbmod [n] {{split} buffer modified}- переход к [n] измененному буферу (sbmod откроет новое окно)
...
### СОХРАНЕНИЕ И ВЫХОД
``` Vimscript 
:w [file] {write} - сохраняет буфер в [file] и остается в текущем редактируемом буфере (default: [file] - редактируемый в данный момент файл)  
...
(\U)/(\L) и (\e)/(\E) - такие же действия как и у \u или \l, только , пока не встретят \e или \E или конец строки замены  
``` 
...
1) :s <=> :s//~/  
2) & <=> :&  
3) Кроме символа "/" в RegExp можно использовать любой неалфавитно-цифровой символ в качестве разделителя, кроме (\, ", |)
``` 
...
Примечание:  
Командой :so импортировать скрипты ex. Таким образом в конфиги импортируются настройки из других файлов/плагинов.  
``` 
...
\- не используются в vim по-умолчанию 
``` Vimscript
